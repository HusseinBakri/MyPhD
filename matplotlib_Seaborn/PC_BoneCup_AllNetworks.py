...
df = pd.read_excel('datasets/PCBoneCupAllNetworks.xlsx', sheetname='Sheet1', header=None)
print(df)
...
Annotations_WithHannibal = ['360 Sprite', '360 Sprite', '500K', '750K', '750K', '750K']
Annotations_WithNoHannibal = ['', '', '1.5M', '1.5M', '1.5M', '1.5M']
...
# set size of figure in inches (for the Thesis)
# Get current size
...
plt.title(r'\textbf{Download \& Processing Times - BoneCup Model' + '\n' + r'\textbf{On PC Across All Networks}')

# Customising ticks labels on X-Axis so they do not look like what is in Excel
plt.xticks(np.arange(6), (
    r'\textbf{GPRS}' + '\n' + r'\textbf{(Simulated)}', r'\textbf{2G}' + '\n' + r'\textbf{Simulated)}',
    r'\textbf{3G}' + '\n' + r'\textbf{(Simulated)}',
    r'\textbf{4G}', r'\textbf{Ethernet}' + '\n' + r'\textbf{JANET}', r'\textbf{CS WiFi}'))
...
New_position_Annotation_X = 4.8  # Moving on X-Axis
...
New_position_Annotation2_X = 4.8  # Moving on X-Axis
...
plt.savefig('datasets/plots/PCBoneCupAllNetworks.png')
plt.savefig('datasets/plots/PCBoneCupAllNetworks_HighDPI.png', dpi=200)
